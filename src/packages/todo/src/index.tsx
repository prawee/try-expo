...
const TodoItem = ({ item }: { item: Schema["Todo"]["type"] }) => (
  <View style={styles.todoItemContainer} key={item.id}>
    <Text
      style={{
        ...styles.todoItemText,
        textDecorationLine: item.isDone ? "line-through" : "none",
        textDecorationColor: item.isDone ? "red" : "black",
      }}
    >
      {item.content}
    </Text>
    ...
    <Button
      onPress={() => {
        client.models.Todo.update(
          { id: item.id, isDone: !item.isDone } as Partial<Schema["Todo"]["type"]> & { id: string },
        );
      }}
      title={item.isDone ? "Undo" : "Done"}
    />
  </View>
);
